...
import { Mail, Lock, Eye, EyeOff, ArrowRight, CheckCircle, Users, Trophy, Star } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleGithub = async () => {
    ...
    try {
      ...
      success('Welcome!', 'Signed in with GitHub.');
      navigate('/dashboard');
    } catch {
      toastError('GitHub sign-in failed', 'Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-nexzen-bg flex relative overflow-y-auto overflow-x-hidden">
      {/* Bg orbs */}
      <div className="absolute -top-40 -left-40 w-96 h-96 rounded-full opacity-20 blur-3xl fixed" style={{ background: 'radial-gradient(circle, #6366F1, transparent)' }} />
      <div className="absolute -bottom-40 -right-40 w-96 h-96 rounded-full opacity-15 blur-3xl fixed" style={{ background: 'radial-gradient(circle, #06B6D4, transparent)' }} />
      <div className="absolute inset-0 bg-grid opacity-40 fixed" />

      {/* Left brand panel */}
      <div className="hidden lg:flex flex-col justify-center px-12 flex-1 relative z-10 pt-20">
        <Link to="/" className="flex items-center gap-2 mb-12 group">
          <img src="/part2.png" alt="Logo" className="w-10 h-10 object-contain transition-all" />
